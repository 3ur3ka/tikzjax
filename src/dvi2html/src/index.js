...
export function dvi2vdom(dviStream, h, ximeraRuleHandler, ximeraPushHandler, ximeraPopHandler, callback) {
    let parser = ximera(title(papersize(html(svg(color(mergeText(dviParser(dviStream))))))));
    let machine = new VDomMachine(h, ximeraRuleHandler, ximeraPushHandler, ximeraPopHandler, callback);
    execute(parser, machine);
    return machine;
}
